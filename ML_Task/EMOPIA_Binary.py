...
def get_data(my_dir, dim):
    ...
    if dim == "Arousal":
        y_col = y_col.replace(['Q1', 'Q2'], 1)
        y_col = y_col.replace(['Q3', 'Q4'], 0)
    else:
        y_col = y_col.replace(['Q1', 'Q4'], 1)
        y_col = y_col.replace(['Q3', 'Q2'], 0)
    y = y_col.values
    x = df.iloc[:, 1:]
    A_selection_Cor = ["Most_Common_Rhythmic_Value", "Number_of_Strong_Rhythmic_Pulses_._Tempo_Standardized", "BPM", "pcm_zcr_sma_skewness", "Note_Density", "Standard_Triads", "Melodic_Large_Intervals", "Strength_Ratio_of_Two_Strongest_Rhythmic_Pulses_._Tempo_Standardized", "Prevalence_of_Dotted_Notes"]
    V_selection_Cor = ["F0env_sma_quartile3", "Minor_Major_Triad_Ratio", "Minor_Major_Melodic_Third_Ratio", "F0env_sma_skewness", "pcm_intensity_sma_minPos", "Similar_Motion", "Strength_Ratio_of_Two_Strongest_Rhythmic_Pulses_._Tempo_Standardized", "Diminished_and_Augmented_Triads", "Dynamic_Range"]
...
if __name__ == "__main__":
    my_dir = os.getcwd()
    dim = "Valence"  # Change to another string to run the experiments for valence
    df, labels_df, y, x, A_selection_index, V_selection_index, all_selection_index = get_data(my_dir, dim)
    average_UAR_all = 0
    average_mean_cm_all = np.zeros((2,2), dtype=np.int)
    average_UAR_V = 0
    average_mean_cm_V = np.zeros((2,2), dtype=np.int)
    average_UAR_A = 0
    average_mean_cm_A = np.zeros((2,2), dtype=np.int)
    average_UAR_SELall = 0
    average_mean_cm_SELall = np.zeros((2,2), dtype=np.int)
    for rand in list(range(5)):
        ...
        for classifier in ['SVM', 'knn', 'RF', 'MLP']:
            ...
            mean_UAR_all = 0
            mean_WAR_all = 0
            mean_cm_all = np.zeros((2,2), dtype=np.int)
            mean_UAR_A = 0
            mean_WAR_A = 0
            mean_cm_A = np.zeros((2,2), dtype=np.int)
            mean_UAR_V = 0
            mean_WAR_V = 0
            mean_cm_V = np.zeros((2,2), dtype=np.int)
            mean_UAR_SELall = 0
            mean_WAR_SELall = 0
            mean_cm_SELall = np.zeros((2,2), dtype=np.int)
            if classifier == "SVM":
